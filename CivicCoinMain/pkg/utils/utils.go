...
// encrypt encripta un mensaje usando la clave pública
func Encrypt(publicKey *rsa.PublicKey, message string) ([]byte, error) {
	...
	encryptedBytes, err := rsa.EncryptOAEP(
		sha256.New(), // Función hash
		rand.Reader,  // Fuente de entropía
		publicKey,    // Clave pública
		messageBytes, // Mensaje a encriptar
		nil,          // Parámetros adicionales (opcional)
	)
	if err != nil {
		return nil, fmt.Errorf("error encriptando el mensaje: %v", err)
	}
	return encryptedBytes, nil
}

// decrypt desencripta un mensaje usando la clave privada
func Decrypt(privateKey *rsa.PrivateKey, encryptedMessage []byte) (string, error) {
	// Desencriptar usando OAEP
	...
	if err != nil {
		return "", fmt.Errorf("error desencriptando el mensaje: %v", err)
	}
